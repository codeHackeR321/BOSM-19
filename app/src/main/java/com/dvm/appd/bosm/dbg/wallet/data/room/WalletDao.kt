...
interface WalletDao {
    ...
    @Query("SELECT itemId, itemName, stallId, price, isVeg, COALESCE(cart_data.quantity, 0) AS quantity FROM stall_items LEFT JOIN cart_data ON itemId = item_id WHERE stallId = :stallId AND isAvailable = :available")
